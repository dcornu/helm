...
/**
  * This is an event sourced entity. It has a state, [[HelmState]], which
  ...
  */
class HelmEntity extends PersistentEntity {

  override type Command = HelmCommand[_]
  override type Event = HelmEvent
  override type State = HelmState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: HelmState = HelmState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case HelmState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        HelmState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class HelmState(message: String, timestamp: String)
...
object HelmState {
  /**
    ...
    */
  implicit val format: Format[HelmState] = Json.format
}

/**
  * This interface defines all the events that the HelmEntity supports.
  */
sealed trait HelmEvent extends AggregateEvent[HelmEvent] {
  def aggregateTag = HelmEvent.Tag
}

object HelmEvent {
  val Tag = AggregateEventTag[HelmEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends HelmEvent
...
/**
  ...
  */
sealed trait HelmCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends HelmCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends HelmCommand[String]
...
/**
  ...
  * application loader.
  */
object HelmSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[HelmState]
  )
}
